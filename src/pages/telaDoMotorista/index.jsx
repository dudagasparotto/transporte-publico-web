...
import { listarRotasDoMotorista } from '../../services/transporte';
...
export default function TelaDoMotorista() {
  ...
  useEffect(() => {
    async function carregarDados() {
      try {
        const [motoristaResp, rotasDoMotorista, avaliacoesResp, mediaResp] =
          await Promise.all([
            api.get(`/motoristas/${id}`),
            listarRotasDoMotorista(id),
        ...
        setErroTela('Nao foi possivel carregar todos os dados do motorista.');
      } finally {
        setCarregando(false);
      }
    }
  ...
  function nomeDaRota(rota) {
    return rota.nome_rota || rota.nome_linha || 'sem nome';
  }

  function classeStatus(status) {
    return status === 'Ativa' ? styles.badge_ativo : styles.badge_inativo;
  }
      ...
      <main className={styles.conteudo}>
        {abaAtiva === 'rotas' && (
          <section className={styles.secao}>
            {erroTela && <p className={styles.erro}>{erroTela}</p>}

            {rotas.length === 0 ? (
              <div className={styles.vazio}>
                <span className={styles.vazioCone}>Rotas</span>
                <p>Nenhuma rota vinculada a este motorista no momento.</p>
              </div>
            ) : (
              <div className={styles.listaRotas}>
                {rotas.map((rota) => (
                  <div key={rota.id_rota} className={styles.cardRota}>
                    <div className={styles.rotaNumero}>
                      {nomeDaRota(rota).slice(0, 2)}
                    </div>
                    ...
                    <div className={styles.rotaInfo}>
                      ...
                      <div className={styles.rotaPercurso}>
                        <span className={styles.parada}>
                          {rota.origem || 'Origem nao cadastrada'}
                        </span>
                      ...
                      <span className={styles.horario}>
                        {rota.quantidade_pontos ?? 0} pontos cadastrados
                      </span>

                      <span
                        className={`${styles.badge} ${classeStatus(
                          rota.status
                        )}`}
                      >
                        {rota.status || 'Ativa'}
